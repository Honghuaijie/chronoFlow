...
1. 进入“系统设置”。
2. 填写飞书自定义机器人 Webhook。
3. 点击“保存”。

预期：

- 页面提示保存成功。
- 页面只显示“已配置”，不回显 Webhook 明文。
- 刷新页面后仍显示“已配置”。
...
1. 在系统设置中确认飞书 Webhook 已配置。
2. 点击“测试发送”。

预期：

- 页面提示测试告警已发送。
- 飞书群收到 ChronoFlow 测试卡片。
...
1. 新建任务并开启“失败告警”。
2. 设置较短超时时间。
3. Glue Shell 填写长时间 sleep 脚本。
4. 手动运行任务。

预期：

- 任务日志最终状态为 `timeout` 或后端定义的超时失败状态。
- 飞书群收到告警卡片。
- 日志详情展示告警状态。

### TC-ALERT-005 未配置 Webhook 时跳过告警

步骤：

1. 在系统设置中清空飞书 Webhook。
2. 创建或编辑任务，开启“失败告警”。
3. 运行一个失败任务。
...
- 任务日志状态为 `failed`。
- 不发送飞书消息。
- 日志详情“失败告警”显示“未发送”，原因包含 Webhook 未配置。

### TC-ALERT-006 Python 脚本失败能触发 failed
...
说明：ChronoFlow 根据进程退出码判断任务失败，不解析日志正文。Python 报错必须让 Shell 也返回非 0。
...
- 任务列表下次运行时间、说明列展示正常。
- 飞书 Webhook 可保存、测试发送，任务 failed / timeout 可触发告警。
